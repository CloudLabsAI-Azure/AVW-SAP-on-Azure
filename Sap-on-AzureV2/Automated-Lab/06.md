...
1. You can observe the **Run** under **28-day run history** tab which is automatically enabled once the flow is turned on.

    ![](media/autorun.png)
    
1. Select **Approvals** from the left-hand side menu and hover the curser on **Request to Access Power BI report** then click on **- [x]** to approve.
