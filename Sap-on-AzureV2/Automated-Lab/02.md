...
In this task, you will review the pipeline that extracts the **SalesOrderHeaders** data from the **SAP Hana database** and copies it to **SalesOrderHeaders** table in Synapse workspace.
...
7. Switch to **Sink (1)** tab and observe the Sink settings of the Pipeline.
   ...
   >**Info:**
     ...
     2. When the pipeline is successful, the data from **HanaIDHeaders** Integration dataset will be copied to the Synapse table that is mapped with the Integration dataset **SynapseIDHeaders**.
...
In this task, you will review the pipeline that extracts the **SalesOrderItems** data from the **SAP Hana database** and copies it to **SalesOrderItems** table in Synapse workspace.
...
4. Switch to **Sink (1)** tab and observe the Sink settings of the Pipeline.
    ...
    ![](../Automated-Lab/media/sinkItems.png)
     ...
     >**Info:** In this Pipeline **SynapseIDOrders (2)** has been selected as the Sink dataset and **PolyBase (3)** as Copy Method. When the pipeline is successful, the data from **HanaIDOrders** Integration dataset will be copied to the Synapse table that is mapped to the Integration dataset **SynapseIDOrders**.
...
In this task, you will review the pipeline that extracts the **Payments** data from the **Cosmos DB** and copies it to **Payments** table in Synapse workspace.
...
3. Navigate to the **Source (1)** tab and observe the Source settings of the Pipeline.
    ...
    >**Info:** In this Pipeline **CosmosDbIDPayments (2)** has been selected as the Source dataset where **CosmosDbIDPayments** is an Integration dataset for the Cosmos database that stores data of Payments that are being fetched from the Storage account.
...
In this task, you will review the pipeline that extracts the **IoT Hub Device** data that is stored in the Storage account then copies it to **iotdatadef** table in Synapse workspace.
...
4. Switch to **Sink (1)** tab and observe the Sink settings of the Pipeline.
    ...
    ![](../Automated-Lab/media/sinkiot.png)
     ...
     >**Info:** 
       ...
       2.  When the pipeline is successful, the data from **StaccID-IoTdata** Integration dataset will be copied to the Synapse table that is mapped to the Integration dataset **SynapseID-IOT**.
