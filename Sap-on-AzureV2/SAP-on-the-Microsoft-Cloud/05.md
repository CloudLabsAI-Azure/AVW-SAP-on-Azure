...
2. On the **aciservicename** endpoint screen, select the **Test** **(1)** tab. Select the editor type as **JSON editor** **(2)**. Replace the contents of the input data with the below-given code **(3)** and select **Test** **(4)**. Verify the **Test result** **(5)**.
   ...
   >**Info :**  
   - Here, you have using the best model to forecast the Payment delay in days for SalesOrderHeaders data..
   - You will use the salesorderHeaders and payments data, you have forecast the payments that have been made for salesorderheaders, and you will build a new view called salespaymentfull.
   - As part of the pre-requist, the SalesPaymentFull View has been already created and published in the Synapse Wokspace.
