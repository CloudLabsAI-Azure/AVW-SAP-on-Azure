...
1. Navigate back to the aiw-sap-iothub-<inject key="DeploymentID" /> resource group and select the Logic App named la-sap-teams-<inject key="DeploymentID" />.
...
1. Under **Method**, select **GET** from the dropdown.
   ...
   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M2-p2-logicapp-45.png?raw=true)
...
1. Select **Authentication** parameter from the dropdown.
   ...
   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M2-p2-logicapp-3.1.png?raw=true)
...
1. Now, select **Add an action** to add a new action to the **For each** control.
...
1. Under **Actions**, scroll down and select **Post message in a chat or channel** action to post a message to Teams.
...
1. After signing into teams, select the following details inside action.
   * Post as: **Flow bot**
   * Post in: **Channel**
...
1. After adding all the values, the final screenshot will look like this:
   ...
   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M2-p2-logicapp-00.png?raw=true)
...
1. Click on **Sign in** and use the below credentials to sign in to your account.
