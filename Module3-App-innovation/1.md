     ...
     1. To add **Apply to each** control, enter the following information:
         ...
         1. Under the **Send an email (V2)** step, enter the following information
             ...
             - Body: Enter the below content:
                ...
                Please provide the confirmation in Teams.
                ```
                - Now we need to add the parameters **Table 1 Product ID value**, **Table 1 Name value** and **Table 1 Pricing value** of **Process and save information from forms** into the body so that data will be fetched and added along with the message. To do so, perform the below steps:
                  ...
                  -  Perform above steps to add other 2 paramters **Table 1 Name value** and **Table 1 Pricing value**.
         ...
         1. Now, enter the following information to configure the **Post an adaptive card to a Teams channel and wait for a response (Preview)** action.
             ...
             3. Message: Clickinside the message box and enter the following content.
                   ```
                {
                ...
                "body": [
                 ...
                 ```
             4. After adding the content, notice that there are few values which are to be replaced in the content with values from the dynamic content list. 
...
1. Now, you will send an email which includes the document to the user account which you added while configuring the **When a new mail arrives** trigger, so that new mail arrives in the inbox and workflow will be triggered.
...
1. Return back to the **Microsoft Teams** app and select **Teams** from the left-hand side menu.
 
1. Notice that you have received a message on the **SAP on Azure** teams channel to **Approve** or **Reject** the add new product request.

1. You can either approve or reject the request.
     -  If you click on **Approve**, the condition you have added in the flow will be true and new product will be added in the SAP system.  
     -  If you **Reject** the request, condition will not be met and product will not added into the SAP system.

1. Click on **Approve** to approve the add new product request.

1. After approving the request, you will receive a confirmation message which is being sent by flow.
...
1. Select the most recent run and review the progress of workflow.

1. Wait until the run status changes to **Succeeded**, then open the flow and review it.

1. Return back to the browser tab in which SAP developer system was open. If you have closed the tab you can navigate to the below URL to open the  SAP developer system.
