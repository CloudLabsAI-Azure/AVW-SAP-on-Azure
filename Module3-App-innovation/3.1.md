...
1. Select **Data** from the left-hand side menu and click on **Connections** from the dropdown list.
...
1. Now, search for **Office 365 Outlook (1)** using the search bar available at the top right corner and select it from the list (2).
...
1. Click on **+New connection** to create a new connection.

1. Now, search for **Microsoft Dataverse (1)** using the search bar available at the top right corner and select it from the list (2).

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-connection-05.png?raw=true)

1. On the **Microsoft Dataverse** pop-up, click on **Create** and use the below credentials to complete the **Sign-in** process and create the connection.
...
1. Now, search for **Microsoft Teams (1)** using the search bar available at the top right corner and select it from the list (2).

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-connection-07.png?raw=true)

1. On the **Microsoft Teams** pop-up, click on **Create** and use the below credentials to complete the **Sign-in** process and create the connection.
...
## Task 2: Import flow in Power Automate to automate the process

In this task, you will import a flow to process information from the document by using an AI model, send the Product Information to the Manager over email for approval and add products to the catalog after the request is approved.


1. Select **My Flows (1)** from the left-hand side menu and click on **Import** to import the workflow.
...
1. Within the Virtual Machine, navigate to this path **C:\LabFiles** and select the **CreateSAPProductFromEmail.zip** file and click on open to choose the file.

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-03.png?raw=true)

1. You will be presented with the below page that provides the information about the package and it's related resources.
...
1. On the **Import setup** blade, enter the following information and select **Save**.

      1. **Setup**: Select **Create a new** from the dropdown.
      2. **Resource name**: Enter `CreateSAPProductFromEmail`

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-06.png?raw=true)

1. Then, click on **Select during import** that is next to **SAP-ODATA-Demo** connector under **Related resources** to select the connector that you created earlier in Exercise 1.
...
1. On the **Import setup** blade, select the **Office 365 Outlook (1)** connection that you created in previous task and click on **Save (2)**.
...
1. On the **Import setup** blade, select the **Microsoft Dataverse (1)** connection that you created in previous task and click on **Save (2)**.
...
1. On the **Import setup** blade, select the **Microsoft Teams** connection that you created in previous task and click on **Save**.
...
1. On the **Import setup** blade, select the **SAP-ODATA-Demo** connection that you created in previous task and click on **Save**.

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-014.png?raw=true)

1. After providing all the information, your screen will look like below screenshot.
...
1. Once the flow is imported, you will be presented with below page. Click on **Open flow** to open the flow.

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-017.png?raw=true)

1. Notice that workflow is opened in a new browser tab, review the all triggers and actions present in the flow.
...
1. Then, expand the  **Extract information from forms** action that is present under **Apply to each** control, click on **X** under **AI model** field to select the model that you created in previous exercise.
...
1. Expand the **Send an email (V2)** action that is present under **Apply to each (2)** control, remove the existing email/username present in the **To** field and enter the below username to send an email of the Product Details to you via Outlook.
   ...
   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-021.png?raw=true)

1. Expand the **Post adaptive card and wait for a response Action** action that is present under **Apply to each (2)** control, remove the existing email/username present in the **Recipient** field and enter the below username to send a message to you on Teams asking for approval to add a new product to the catalog.

      * Email/Username: <inject key="AzureAdUserEmail"></inject>

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-022.png?raw=true)
...
1. Select **My Flows** from the left-hand side menu and select the **CreateSAPProductromEmail** flow you created in the previous steps.

   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-46.png?raw=true)

1. Select **more ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-Ex1-flow-024.png?raw=true)** option and then click on **Turn on** to make the flow available to use.
