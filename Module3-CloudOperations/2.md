...
1. For **Subnet**field, select **default** subnet from the dropdown.
...
1. Select **Secrets** from the left-hand side menu of key vault.
...
1. For **IP address** field, enter the **Public IP Address** of the SAP HANA VM that you copied earlier.
...
1. Once the **SAPHANAProvider** is created, it will be listed under providers.
   ...
   > Note: If you are not able the view the provider on the Providers pane, click on Refresh to fetch the recent results.
...
1. You can see the detailed utilization of the **Memory** on HANA and MAX touched so far on the **Utlization** tab.
...
1. On the **Manage actions** blade, click on **+ New action group** to create a action group.
...
1.Review the informtaion you have entered and click on **Create**.
...
1. On the **Select a resource** blade, enter the following information:
    ...
    3. Select the log analytics worskpace named sapmon-log-UniqueId.
...
1. On the **Select aSignal** blade, select **Custom log search**.
...
1. Enter the value **0** for **Threshold value** paramter.
