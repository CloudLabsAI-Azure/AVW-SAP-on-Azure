...
In this task, you will review the recovery service vault resource that has been pre-deployed and is being used to store the recovery points that are created over time.

1. Navigate back to the browser tab in which Azure Portal is open and select **SAPHANARG** resource group from the list.

1. Select the **Recovery Service Vault** resource named **SAPHANARecoveryvault**.

1. On the **Overview** blade of **SAPHANARecoveryvault**, review the information about the Azure Monitor.

## Task 2: Configure Private Endpoints for Backup

## Task 2.1: Verify the status of Managed Identity for the vault

In this task, you will verify the status of managed identity for the recovery service vault that allows it to create and use private endpoints that are required for backup.
...
1. On the **Private endpoint connections** blade, verify that **Backupendpoint** is listed.
...
In this task, you will run an on-demand backup for the SAP HANA database running on an Azure Virtual Machine.
