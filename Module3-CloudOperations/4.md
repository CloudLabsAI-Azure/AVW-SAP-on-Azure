...
In this task, you will restore the SAP HANA databases running on an Azure Virtual Machine (VM), which the Azure Backup service has backed up to a Recovery Services vault.
 
> **Note**: This task will be read-only, you will not be able to restore the databases running on the SAP HANA VM from the recovery service vault as the databases are already restored in the provided lab environment. 
