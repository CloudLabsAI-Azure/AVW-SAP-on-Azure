...
1. Copy the command from notepad and run it in the SAP HANA VM to switch to the system administator user account of the SAP HANA database.
...
1. Now, copy the below command to notepad and replace the following values.
   ...
   ```
     ...
     2. **<hostname of SAP HANA VM>**: Enter the hostname of the virtual machine that has SAP HANA database installed in it.
...
1. Next, copy the command from notepad and run it in the SAP HANA VM to create the key.
   
   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-p2-Ex4-sapvm-3.png?raw=true)   
   
   > Info: The key will be used to log into the HANA system and create a backup user (AZUREWLBACKUPHANAUSER) in the HANA system then assign reuired permissions to the user to perform backup.  
...
1. Run the below command to logout from the database.
...
1. Replace the **SID**, **<InstanceNumber>** and **<Key Name>** values in the below command and run it to execute the pre-resgistration script.
...
1. Once the script execution is done, you will be presented with below pane.
   
   ![](https://github.com/CloudLabsAI-Azure/AIW-SAP-on-Azure/blob/main/media/M3-p2-Ex4-sapvm-7.png?raw=true)
   
1. Minimize the putty session to the SAP HANa VM and naviagate back to the browser tab in which Azure portal is open.
...
1. Review the listed databases and select the database on the **Backup Items (SAP HANA in Azure VM)** blade.
   ...
   > Notice that the status of the backup items is **Warning(Initial backup pending)**, this is because you have configured the backup but didn't the run on-demand backup.
...
1. From the **Backup Jobs** blade, you can monitor the progress of the backup job.
   ...
   > You can click on **Refresh** button on the **Backup Jobs** blade, to fetch the recent results.
...
1. Check the box which is next to **Overwrite if the DB with same name already exists on selected HANA instance** so that if the database with same name exists that will overwritten. 
