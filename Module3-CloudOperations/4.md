...
In this exercise, you will review the pre-deployed Recovery Services vault that is being used to store the recovery points that are created over time. You will then review the pre-created private endpoints that are required for performing the backup. Then, you will review the SAP HANA databases that are being backed up to the recovery service vault running on an Azure Virtual Machine. You will also review the SAP HANA database that is restored from the recovery service vault.
