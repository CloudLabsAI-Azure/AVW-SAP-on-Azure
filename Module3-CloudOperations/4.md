...
In this exercise, you will create a Recovery Services vault to store the recovery points that are created over time. You will create private endpoints required for performing the backup. Then, you will configure and enable backup for the SAP HANA database running on an Azure Virtual Machine. You will also run an on-demand backup for the SAP HANA database and restore it.

This exercise includes the following tasks:

  * Create Recovery Service Vault
  * Configure Private Endpoints for Backup
  * Discover databases
  * Configure backup
...
1. Navigate back to the **SAPHANARG** resource group and select the **Recovery Service Vault** resource named **SAPMonitor**.
...
1. In the above steps you have reviewed the pre-deployed recovery service vault, you can follow the below instructions to create a new recovery service vault.
   ...
   > **Note**: Below instructions are read-only, as the provided lab environment doesn't have required permissions to create the recovery service vault resource.
...
## Task 2.2: Create Private Endpoints for Azure Backup
...
1. Navigate to the **Resource Group** and select the recovery service vault named **SAPHANARecoveryvault**.
...
1. Now, on the verify that the **Backupendpoint** is listed on the **Private endpoint connections** blade.
...
## Task 3: Discover the databases

In this task, you will discover all SAP HANA databases on the Virtual Machine.

> **Note**: This task will be read-only, you will not be able to the discover the databases running on the SAP HANA VM as the database discovery is already done in the provided lab environment. 
...
> **Note**: This task will be read-only, you will not be able to the congigure backup for the databases running on the SAP HANA VM as backup is alreadyconfigured for the datbases in the provided lab environment. 
...
1. Review the listed databases that are backed up and their status on the **Backup Items (SAP HANA in Azure VM)** blade.
...
1. In the above steps you have reviewed the databases that are backed up, you can follow the below instructions to configure backup for the discovered databases running on SAP HANA Virtual Machine.    
   ...
   > **Note**: Below instructions are read-only, as the databases running on the SAP HANA Virtual Machine are already backed up to recovery service vault in the provided lab environment.
...
In this exercise, you have covered the following:
  ...
  * Created Recovery Service Vault to store the recovery points that are created over time.
  * Configured Private Endpoints required for performing the Backup.
  * Configure backup for the SAP HANA database running on an Azure Virtual Machine.
  * Run on Demand backup for the SAP HANA database running on an Azure Virtual Machine.
  * Restore DB
