...
1. Click on the **aiw-sap-SUFFIX** Resource group and confirm whether you have all the below resources deployed successfully.
   ...
   * **IoT Hub**: You will be using the existing iothub and create an iothub device then view the data generated by iothub device.

   * **Service Bus**: You will be using the Serive bus to store messages in a Serive bus queue.

   * **Logic App**: You will be using the logic app to create a workflow which monitors the queue and send an e-mail notification whenever a message is added to the queue.
